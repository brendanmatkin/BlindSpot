...
int error = 20;            // # of pixels to ignore before detect - improves margin of error!

boolean refresh = false;      // detect if there are too many colored pixels
int refreshTest = 500000;     // 1300000 - 2cam,60fps. how many blue pixels to detect before refresh
...
int camFR = 5;          // cam Frame Rate
int numCam = 6;          // number of cameras
...
void setup() {
  ...
  if (cameras.length == 0) {
    ...
    cam1 = new Capture(this, camW, camH, "WideCam 320 #3", camFR);  // these names must be defined in the registry
    cam2 = new Capture(this, camW, camH, "WideCam 320 #2", camFR);
    cam3 = new Capture(this, camW, camH, "WideCam 320 #4", camFR);
    cam4 = new Capture(this, camW, camH, "WideCam 320 #5", camFR);
    cam5 = new Capture(this, camW, camH, "WideCam 320 #6", camFR);
    cam6 = new Capture(this, camW, camH, "WideCam 320 #1", camFR);
...
void draw() {
  ...
  if (cam1.available()) {
    ...
    if (keyPressed == true || v_toggle1 == 1.0f || refresh == true) {   // manual or pixel count based auto-refresh (may also use refresh variable for external manual-refresh ie. a button)
      ...
      background.set((camW*2), 0, cam3);
      background.set((camW*3), 0, cam4);
      background.set((camW*4), 0, cam5);
      background.set((camW*5), 0, cam6);
